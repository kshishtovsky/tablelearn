...
/*amqp.connect('amqp://localhost', (error0, connection) => {
 ...
 connection.createChannel((error1, channel) => {
  ...
  channel.assertQueue('', {
   ...
   const getDialogsId = "112"
   const postDialogsId = "111"

   app.get('/dialogs', (req, res, next) => {
    channel.consume(q.queue, (msg) => {
     if (msg.properties.correlationId == getDialogsId) {
 ...
 connection.createChannel((error1, channel) => {
  ...
  app.post('/dialogs', (req, res) => {
   channel.assertQueue('', {
    ...
    channel.sendToQueue('dialogs__post', Buffer.from(JSON.stringify(req.body)), {
     correlationId: correlationId,
     replyTo: q.queue
    })
   })
